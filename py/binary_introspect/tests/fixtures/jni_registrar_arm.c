/* ELF 32-bit ARM fixture for generic-first JNI method discovery.
 *
 * Exercises two specification-defined registration mechanisms on the 32-bit
 * ARM AAPCS calling convention (JNIEnv* in r0, jclass in r1,
 * JNINativeMethod* in r2, jint nMethods in r3):
 *   1. a static JNINativeMethod[] table passed to RegisterNatives, and
 *   2. a Java_* exported symbol.
 *
 * Built by fixtures/build.sh with arm-linux-gnueabi-gcc (or `zig cc -target
 * arm-linux-gnueabi`). See that script for the exact command. The built
 * libjni_registrar_arm.so is committed so the pytest suite runs without a
 * cross toolchain.
 */

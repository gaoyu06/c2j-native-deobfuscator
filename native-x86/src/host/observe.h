 ...
 *
 * Given a target process the invoking user owns, this enumerates loaded
 * modules and, for exports a plugin asked to watch, reports symbol records.
 * The optional Linux live pass also reports entry/return call-site records.
 * It reports *program structure* only: module bases, symbol names and
 * addresses, and (where supported) control-flow edges. It never reads
 * argument registers, buffer contents, return values or keys.
 *
 ...
 *
 * Linux and Windows implementations live in observe_linux.c and
 * observe_windows.c. Other platforms get observe_stub.c, which fails
 * honestly.
 */
...
typedef struct nx86_observe_config {
    ...
    int      allow_live;      /* request Linux live entry/return observation */
 ...
 * not exist or cannot be inspected. On platforms with a scalar uid,
 * *out_owner_id receives it when non-NULL; otherwise it is set to zero. */
...
/*
 * Inspect cfg->pid and emit records through `bus`:
 ...
 * target cannot be inspected at all. Diagnostic detail is
